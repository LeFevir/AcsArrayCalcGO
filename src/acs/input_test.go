...
func TestNewArrayFromFile(t *testing.T) {
	s := NewArrayFromFile("test_files\\hand_array.txt")
	if s.Name != "Hand-Gavrilov" {
		t.Log("A mistake at reading array name!")
		t.Fail()
	}
	if s.Aperture != 0.170 {
		t.Log("A mistake at reading array aperture!")
		t.Fail()
	}
	if s.CurvatureRadius != 0.130 {
		t.Log("A mistake at reading array CurvatureRadius!")
		t.Fail()
	}
	if s.ElementRadius != 0.0035 {
		t.Log("A mistake at reading array ElementRadius!")
		t.Fail()
	}
